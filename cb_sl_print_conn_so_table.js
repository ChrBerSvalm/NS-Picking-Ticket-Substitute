 ...
 */
define(['N/ui/serverWidget', 'N/search', 'N/log', 'N/format', 'N/record'], 
(ui, search, log, format, record) => {

  const PARAM_SO_ID = 'soId';
  const PARAM_IR_ID = 'irId';
  ...
  const onRequest = (ctx) => {
    try {
      ...
      if (ctx.request.method === 'GET') {
        ...
        const irId = rawParams.irId || rawParams['amp;irId'] || '';
        log.debug('onRequest - Params', { soId, irId, rawParams });
        
        let soNumber = soId ? loadSONumber(soId) || '' : '';
        let irNumber = irId ? loadIRNumber(irId) || '' : '';
        let todayStr = '';
        ...
        const form = ui.createForm({ 
          title: soId ? 'Connected Sales Order Lines' : (irId ? 'Connected Work Order Lines' : 'Connected Transaction Lines')
        });
        ...
        form.addField({
          ...
          (irId ? '<div>Transaction: ' + escapeHtml(irNumber || irId) + '</div>' : '') +
          '</div>';
          ...
          const printHeaderHtml =
            '<div id="print-header">' +
              ...
              '<div class="meta" style="font-size:12pt;color:#222;line-height:1.45;">' +
                ...
                (irId ? '<div><strong>Transaction:</strong> ' + escapeHtml(irNumber || irId) + '</div>' : '') +
              '</div>' +
            '</div>';
        ...
        var workOrder = irNumber.substring(0,2);
        // Sublist
        ...
        if (workOrder === 'WO') {
          sub.addField({ id: 'custpage_workorder', type: ui.FieldType.TEXT, label: 'Kontroll antal' });
          sub.addField({ id: 'custpage_workorder2', type: ui.FieldType.TEXT, label: 'Kontroll mått' });
          sub.addField({ id: 'custpage_workorder3', type: ui.FieldType.TEXT, label: 'Signatur' });
        }

        if (soId) {
          let irItemsSet = new Set(irId ? loadIRItems(irId) : []);
          const lines = loadSalesOrderLines(soId);

          lines.forEach((ln, i) => {
            const isIRItem = irId ? irItemsSet.has(String(ln.itemId)) : false;
...
function loadSalesOrderLines(soId) {
  ...
  soLineSearch.run().each(r => {
    ...
    const qtyOrdered = Number(r.getValue('quantity')) || 0;

    log.debug('SO Line', { lineNum, itemId, itemText, itemType, qtyOrdered });

    if (itemType === 'Kit' || itemType === 'Kit/Package') {
      log.debug('Kit detected', { itemId, itemText, qtyOrdered });

      try {
        const members = getKitMembers(itemId);
        log.debug('Kit members returned', { count: members.length, members });

        members.forEach(m => {
          results.push({
            lineNum: lineNum + ' (kit member)',
            itemId: m.memberId,
            itemText: '   → ' + m.memberText,
            displayName: '',
            qtyOrdered: qtyOrdered * m.memberQty,
            qtyCommitted: null,
            qtyPicked: null,
        ...
        log.error('Kit expansion failed', { itemId, error: e });
      }
...
function getKitMembers(kitItemId) {
  ...
  function loadIRNumber(irId) {
    try {
      const res = search.create({
        type: 'itemreceipt',
        filters: [['internalid', 'anyof', irId], 'AND', ['mainline', 'is', 'T']],
        columns: [ search.createColumn({ name: 'tranid' }) ]
      }).run().getRange({ start: 0, end: 1 });
      if (res && res.length) return res[0].getValue('tranid') || '';
      const resWO = search.create({
        type: 'workorder',
        filters: [['internalid', 'anyof', irId], 'AND', ['mainline', 'is', 'T']],
        columns: [ search.createColumn({ name: 'tranid' }) ]
      }).run().getRange({ start: 0, end: 1 });
      if (resWO && resWO.length) return resWO[0].getValue('tranid') || '';
      return '';
    } catch (e) {
      log.error('loadIRNumber error', e);
      return '';
    }
  }
  ...
  function loadIRItems(irId) {
    const items = [];
    const irSearch = search.create({
      type: 'transaction',
      filters: [['type', 'anyof', ['ItemRcpt','WorkOrd']], 'AND', ['internalid', 'anyof', irId], 'AND', ['mainline', 'is', 'F'], 'AND', ['item', 'noneof', '@NONE@']],
      columns: [ search.createColumn({ name: 'item' }) ]
    });
    irSearch.run().each(r => { items.push(String(r.getValue('item'))); return true; });
    return items;
  }
